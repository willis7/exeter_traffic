...
for year in years:
    # import my normalised coordinate data for just inner data
    road_coords = pd.read_csv("../../data/road_coords_norm_vals_in%s.csv"%year, header = 0)
    ...
    for i,m in enumerate(mode):
        ...
        for CP in road_coords["CP"].unique():
            ...
            map.plot(x, y, 'o', color = colors[i], markersize=markersize*150)

        plt.title('Relative AADF for %s %s Inner Roads'%(m,year))    

        fig = plt.gcf()
        
        fig.savefig('../../plots/maps/%s/Exeter_%s%s_inner.png'%(year, m,year), dpi=100)
