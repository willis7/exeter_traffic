...
# Only making the NoCars for 2014 for the presentation
years = [2014]
...
for year in years:
    ...
    mode = ['PedalCycles', 'Motorcycles', 'BusesCoaches', 'LightGoodsVehicles', 
           'AllHGVs', 'AllMotorVehicles']
    
        
    # make a dataframe of just the columns we want to stack in the bar plot
    exeter_year = exeter_year[["Road","PedalCycles","Motorcycles","BusesCoaches",
    ...
    plt.title('Road useage by non-car vehicle type (CPs) %s'%str(year))
    
    fig = plt.gcf()

    fig.savefig('../../plots/barplots/NoCars/eachCP_%s.png'%str(year))
    ...
    plt.title('Inner Road useage by non-car vehicle type %s'%str(year))
    
    fig = plt.gcf()
    fig.savefig('../../plots/barplots/NoCars/inner_barplot%s.png'%str(year))
    ...
    plt.title('Outer Road useage by non-car vehicle type %s' %str(year))
    
    fig = plt.gcf()
    fig.savefig('../../plots/barplots/NoCars/outer_barplot%s.png'%str(year))
    
    # Group all roads
    group_year = exeter_year.groupby(exeter_year.index).mean()
    
    group_year.plot.bar(stacked = True)
    plt.title('All Road useage by non-car vehicle type %s' %str(year))
    fig = plt.gcf()
    fig.savefig('../../plots/barplots/NoCars/each_road_barplot%s.png'%str(year))
