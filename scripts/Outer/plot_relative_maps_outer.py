...
for year in years:
    ...
    road_coords = pd.read_csv("../../data/road_coords_norm_vals_out%s.csv" % year, header=0)
    ...
    for i, m in enumerate(mode):
        ...
        plt.title('Relative AADF for %s %s Outer Roads' % (m, year))

        fig = plt.gcf()

        fig.savefig('../../plots/maps/%s/Exeter_%s%s_outer.png' % (year, m, year), dpi=100)
